...
from .validators import phone_validation, email_validation
...
def login_view(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            user = CustomModelBackend().authenticate(request, username=username, password=password)
...
def signup_view(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            phone = form.cleaned_data.get("phone", '0911111111')
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            if not email_validation(email):
                form.add_error('email', 'ایمیل معتبر نمی‌باشد.')
